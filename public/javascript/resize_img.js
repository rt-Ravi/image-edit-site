...
const width_tag = document.querySelector(".width");
const height_tag = document.querySelector(".height");
...
async function check(e){
    if(imgsize >= 11 || !inputImage || (!imgtype.includes("png") && !imgtype.includes("jpg") && !imgtype.includes("jpeg") && !imgtype.includes("tiff") && !imgtype.includes("svg") && !imgtype.includes("bmp")) ){
      ...
      const width = width_tag.value;
      const height = height_tag.value;
      const formData = new FormData();
      formData.append("file", inputImage);
      formData.append("name", image_type);
      formData.append("width", width);
      formData.append("height", height);
      const res = await fetch("/change-format-and-resize-image-width-height", {
        method: "POST",
        headers: {
      ...
      if(data){
          ...
          h1_tag.textContent = "Image Resized Succesfuly";
